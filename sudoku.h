...
//Convierte el sudoku recibido en un tablero donde se almacenas las posibilidades de cada celda
...
void regionFilter(Board& b, unsigned int i, unsigned int j) {
    assert(!assigned(b,i,j)); //Si la celda no esta asignada hace todo, de lo contrario aborta el programa
...
bool solution(const Board& b){
    return countAssigned(b)==81;
}
...
//Retorna verdadero si hay un fallo en una celda(la celda se quedó sin posibilidades)
...
//Encuentra la celda sin asignar con menos posibilidades, almacena su posicion por referencia en i y j
